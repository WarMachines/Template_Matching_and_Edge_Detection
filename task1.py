...
def parse_args():
    ...
    parser.add_argument(
        "--img_path", type=str, default="data/Acura 2004 Touring Large Luxury Crossover SUV.jpg",
        ...
        for j in range(1,len(padded_img[0])-1):
            ...
            for xindex in range(-1,2):
                for yindex in range(-1,2):
                    value = value + (padded_img[i+xindex][j+yindex])*(kernel[1+xindex][1+yindex])
            row.append(value)
...
def normalize(img):
    ...
    """
    # TODO: implement this function.
    min_pixel_value = img[0][0]
    max_pixel_value = img[0][0]
...
def edge_magnitude(edge_x, edge_y):
    ...
    for i in range(0,len(edge_x)):
        row = []
        for j in range(0,len(edge_x[0])):
            new_pixel_value = np.sqrt((edge_x[i][j] * edge_x[i][j]) + (edge_y[i][j] * edge_y[i][j]))
            row.append(new_pixel_value)
